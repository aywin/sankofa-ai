...
export function DiscoveryCard({ result }: { result: DiscoveryResult }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-neutral-200 transition hover:border-emerald-300 hover:bg-emerald-50/50 dark:border-neutral-800 dark:hover:border-emerald-800 dark:hover:bg-emerald-950/40">
      <Link href={`/plants/${result.taxonSlug}`} className="block">
        {result.photoUrl && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={result.photoUrl} alt={result.nomPrincipal} className="h-32 w-full object-cover" />
        )}
      ...
      <div className="flex items-center justify-between gap-2 p-4 pt-3">
        {result.attribution ? (
          <SourceCard source={result.attribution} variant="compact" />
        ) : (
          <span />
        )}
        <Link
